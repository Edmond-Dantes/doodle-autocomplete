  ...
  const generatePredictedShape = useCallback(
    async (label: string, shape: TLUnknownShape | TLDrawShape) => {
      ...
      const shapeTypeMap: Record<string, string> = {
        ...
        triangle: "moon", // should be "triangle" but just testing other shapes for now
        axis: "axis",
        bat: "bat",
        car: "car",
        cat: "cat",
      ...
      if (label !== "unknown" && label in shapeTypeMap) {
        ...
        if (bounds) {
          editor.run(() => {
            ...
            editor.deleteShapes([ (shape as any).id ]) // ← use the id Louise updated
            //editor.deleteShapes([shape]); // Edmond original
